...
CREATE VIEW v_polisy_indywidualne_wlascicieli AS
...
CREATE VIEW v_polisy_indywidualne AS
...
CREATE VIEW v_polisy_przekroczona_wartosc AS
...
create view v_agenci_wskazniki AS
WITH
...
,NVL(round(akt_polisy.ilosc/wszystkie_polisy.ilosc*100,1),0) || '%' AS wsk_akt_polis FROM agenci
LEFT JOIN wszystkie_polisy using (nr_agenta)
LEFT JOIN akt_polisy using  (nr_agenta)
;

CREATE VIEW v_szkody_przeterminowane AS
SELECT nr_polisy,data_do WA½NO—_POLISY, data_zgloszenia, trunc(data_zgloszenia-data_do) dni_przekroczenia FROM szkody
INNER JOIN polisy USING (nr_polisy)
WHERE data_zgloszenia>data_do
;

CREATE VIEW v_szkody_wysokie AS
...
inner join osoby o using (id_osoby)
...
WITH
    ...
    ,w2 AS (select sum(suma_ubezpieczenia) suma from polisy p where exists (SELECT 1 FROM szkody s WHERE p.nr_polisy = s.nr_polisy))
SELECT 
